...
SALAMANCA_LAT = 40.97
SALAMANCA_LON = -5.66
STEPS = list(range(6, 169, 6))   # 6, 12, …, 168 hours
...
def _compute_daily_stats(
    ...
    Returns
    -------
    dates      : list of ``datetime.date`` objects (one per day)
    day_labels : formatted strings for the plot x-axis
    ...
    for day in range(1, n_days + 1):
        # 6-hourly steps falling within this 24-hour window (e.g. 6,12,18,24 for day 1)
        day_steps = list(range((day - 1) * 24 + 6, day * 24 + 1, 6))
        available = {sh: np.array(step_data[sh]) for sh in day_steps if sh in step_data}

        if len(available) < 2:
            log.warning("Day %d: only %d step(s) available, skipping.", day, len(available))
            continue
        ...
        arr = np.array([available[sh][:n_mem] for sh in sorted(available)])  # (steps, members)
...
def build(csv_path: Path, json_path: Path, png_path: Path) -> None:
    ...
    try:
        for fc_type in ("cf", "pf"):
            fd, tmp_path = tempfile.mkstemp(suffix=f"_{fc_type}.grib2")
            os.close(fd)
            tmp_files.append(tmp_path)
            log.info("Downloading ENS type=%s …", fc_type)
            client.retrieve(type=fc_type, param="2t", step=STEPS, target=tmp_path)
            log.info("Parsing %s …", tmp_path)
            parsed = _collect_ens_t2m(tmp_path)
            if not parsed:
                log.warning("No t2m data found in %s (type=%s)", tmp_path, fc_type)
